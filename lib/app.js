...
const MAPPINGS = {
  ...
  developerId: {
    ...
    fun: (devUrl) => ( devUrl ? devUrl.split('id=')[1] : '')
  },
  ...
  screenshots: {
    path: ['ds:5', 0, 12, 0],
    fun: (screenshots) => {
      if (screenshots === null || screenshots === undefined) return [];
...
function normalizeAndroidVersion (androidVersionText) {
  if(androidVersionText || androidVersionText == null || androidVersionText == undefined || androidVersionText.length <= 0) {
    return '0';
  }
